...
var (
	...
	// VolumePrefix is the prefix of Windows Volume Unique ID
	VolumePrefix = "\\\\?\\Volume{"
)
...
func (VolumeAPI) MountVolume(volumeID, path string) error {
	...
	var status string
	result, err := part.InvokeMethodWithReturn("AddAccessPath", path, nil, &status)
	if result != 0 || err != nil {
		return fmt.Errorf("error mount volume (%s) to path %s. result %d, status %s, error: %v", volumeID, path, result, status, err)
	}
...
func (VolumeAPI) UnmountVolume(volumeID, path string) error {
	...
	result, err := part.InvokeMethodWithReturn("RemoveAccessPath", path)
	if result != 0 || err != nil {
		return fmt.Errorf("error umount volume (%s) from path %s. result %d, error: %v", volumeID, path, result, err)
	}
	return nil
}
...
func (VolumeAPI) GetVolumeIDFromTargetPath(mount string) (string, error) {
	...
	size := len(outPathBuffer)
	err := windows.GetVolumePathName(utf16FullMountPath, &outPathBuffer[0], uint32(size))
	if err != nil {
		return "", err
	}

	targetPath := windows.UTF16PtrToString(&outPathBuffer[0])
	...
	outPathBuffer = make([]uint16, windows.MAX_LONG_PATH)
	size = len(outPathBuffer)
	err = windows.GetVolumeNameForVolumeMountPoint(utf16TargetPath, &outPathBuffer[0], uint32(size))
	if err != nil {
		return "", err
	}
