...
var (
	...
	notMountedFolder = errorspkg.New("not a mounted folder")
)
...
func (volumeAPI) MountVolume(volumeID, path string) error {
	...
	if err != nil {
		if errorspkg.Is(windows.GetLastError(), windows.ERROR_DIR_NOT_EMPTY) {
...
func findClosestVolume(path string) (string, error) {
	...
	for i := 0; i < 256; i++ {
		...
		if isSymlink || mountedFolder {
			...
			if err != nil && !errorspkg.Is(err, notMountedFolder) {
				return "", err
			}
...
func isMountedFolder(path string) (bool, error) {
	...
	for {
		...
		if err != nil {
			if !errorspkg.Is(err, windows.ERROR_NO_MORE_FILES) {
				return false, err
			}
			break
		}
